...
class Services implements ServicesInterface
{
	public function registerServices($services)
	{
		$services['user.subscription.create'] = function($c) {
			return new \Message\Mothership\User\Subscription\Create($c['db.query'], $c['user.current']);
		};
		...
		$services['user.subscription.delete'] = function($c) {
			return new \Message\Mothership\User\Subscription\Delete($c['db.query'], $c['user.current']);
		};
	}
}